...
void hardmode::loadTowerPositions()
{
    QPoint positions[]=
    {
        QPoint(180,580),
...
void hardmode::mouseDoubleClickEvent(QMouseEvent *e){
...
void hardmode::drawWave(QPainter *painter)
{
...
void hardmode::awardGold(int gold)
{
    m_playrGold += gold;
    update();
}

void hardmode::addWayPoints()
{
...
void hardmode::getHpDamage(int damage/* = 1*/)
{
...
void hardmode::removedEnemy(enemy *enemy)
{
    Q_ASSERT(enemy);
...
void hardmode::removedBullet(bullet *bullet)
{
    Q_ASSERT(bullet);
...
void hardmode::addBullet(bullet *bullet)
{
    Q_ASSERT(bullet);
...
bool hardmode::loadWave()
{
    ...
    if(m_waves<3) {
        int time[]={100,3000,5000,7000,9000,12000,15000,18000};
        for(int i=0;i<8;++i){
    ...
    if(m_waves==9){
        enemy *enemy=new class enemy(startpoint,this,4);
        m_enemyList.push_back(enemy);
        QTimer::singleShot(100,enemy,SLOT(doActivate()));
    }
    return true;
...
QList<enemy *> hardmode::enemyList() const
{
    return m_enemyList;
}
